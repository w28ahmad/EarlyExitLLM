...
def resolve_local_path(path):
    """If path is a HF hub ID (not a local dir), download and return the local snapshot path."""
...
class EEModel(nn.Module):
    ...
    def from_pretrained(
        ...
        for layer_idx in range(len(base_model.model.layers)):
            pth = torch.load(predictor_path+'/model'+str(layer_idx)+'.pth', map_location=base_model.device, weights_only=False)
            if isinstance(pth, dict):  # state_dict from train_mlp.py
                m = MLP(input_size=12, hidden_size=512, output_size=1)
                m.load_state_dict(pth)
                pth = m
            predictors.append(pth.to(base_model.device).to(kwargs.get('torch_dtype', torch.float32)))
        base_model.model.predictors = predictors
        base_model.model.pred_thresholds = pred_thresholds
        ea_model_local_path = resolve_local_path(ea_model_path)
        configpath=os.path.join(ea_model_local_path,"config.json")
        model = cls(
            base_model,
            base_model_path,
            configpath,
        )
        load_model_path=os.path.join(ea_model_local_path, "pytorch_model.bin")
        ea_layer_state_dict = torch.load(load_model_path,
                                         map_location=base_model.device, weights_only=False)
